# PSBT Howto for Mytherra Core

Since Mytherra Core 0.17, an RPC interface exists for Partially Signed Mytherra
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/mytherra/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Mytherra transactions that are not fully signed
...
#### Multisig with multiple Mytherra Core instances
...
Mytherra Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `mytherra-cli` is used.
...
Later, when *V* MYT has been received on *Amulti*, and Bob and Carol want to
