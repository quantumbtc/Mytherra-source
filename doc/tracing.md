# User-space, Statically Defined Tracing (USDT) for Mytherra Core

Mytherra Core includes statically defined tracepoints to allow for more
...
**Note**: Mytherra Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Mytherra Core
...
Multiple tools can list the available tracepoints in a `mytherrad` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Mytherra Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/mytherrad
stap net        outbound_message 0x0000000000107c05 /src/mytherrad
stap validation block_connected  0x00000000002fb10c /src/mytherrad
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Mytherra Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/mytherrad | grep NT_STAPSDT -A 4 -B 2
