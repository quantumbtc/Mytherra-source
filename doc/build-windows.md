...
Below are some notes on how to build Mytherra Core for Windows.

The options known to work for building Mytherra Core on Windows are:
...
Note that for WSL the Mytherra Core source path MUST be somewhere in the default mount file system, for
example /usr/src/mytherra, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\mytherra`, for example:

    make install DESTDIR=/mnt/c/workspace/mytherra
