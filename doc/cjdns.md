# CJDNS support in Mytherra Core

It is possible to run Mytherra Core over CJDNS, an encrypted IPv6 network that
...
done outside of Mytherra Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Mytherra network and individual nodes.
...
before it will work with your Mytherra Core node. This is described in steps
...
## Run Mytherra Core with CJDNS

Once you are connected to the CJDNS network, the following Mytherra Core
...
When enabled, this option tells Mytherra Core that it is running in an
...
IPv6 local network. This helps Mytherra Core perform better address management:
...
CJDNS support was added to Mytherra Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `mytherra-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/mytherra/mytherra/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Mytherra Core

There are several ways to see your CJDNS address in Mytherra Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `mytherra-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `mytherra-cli getpeerinfo`).
