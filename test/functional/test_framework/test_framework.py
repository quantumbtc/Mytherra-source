...
TMPDIR_PREFIX = "mytherra_func_test_"
...
class MytherraTestMetaClass(type):
    """Metaclass for MytherraTestFramework.

    Ensures that any attempt to register a subclass of `MytherraTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'MytherraTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("MytherraTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("MytherraTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class MytherraTestFramework(metaclass=MytherraTestMetaClass):
    """Base class for a mytherra test script.

    Individual mytherra test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave mytherrads and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop mytherrads after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use mytherra-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_mytherrad = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "mytherrad" + config["environment"]["EXEEXT"],
        )
        fname_mytherracli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "mytherra-cli" + config["environment"]["EXEEXT"],
        )
        fname_mytherrautil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "mytherra-util" + config["environment"]["EXEEXT"],
        )
        self.options.mytherrad = os.getenv("MYTHERRAD", default=fname_mytherrad)
        self.options.mytherracli = os.getenv("MYTHERRACLI", default=fname_mytherracli)
        self.options.mytherrautil = os.getenv("MYTHERRAUTIL", default=fname_mytherrautil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: mytherrads were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("MytherraRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'mytherrad', self.options.mytherrad) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'mytherra-cli', self.options.mytherracli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a mytherrad test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple mytherrad test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as mytherrad's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    mytherrad=self.options.mytherrad,
                    mytherra_cli=self.options.mytherracli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in mytherra.conf
    ...
    def skip_if_no_mytherrad_tracepoints(self):
        """Skip the running test if mytherrad has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("mytherrad has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_mytherrad_zmq(self):
        """Skip the running test if mytherrad has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("mytherrad has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if mytherra-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("mytherra-wallet has not been compiled")

    def skip_if_no_mytherra_util(self):
        """Skip the running test if mytherra-util has not been compiled."""
        if not self.is_mytherra_util_compiled():
            raise SkipTest("mytherra-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if mytherra-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("mytherra-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether mytherra-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether mytherra-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_mytherra_util_compiled(self):
        """Checks whether mytherra-util was compiled."""
        return self.config["components"].getboolean("ENABLE_MYTHERRA_UTIL")
