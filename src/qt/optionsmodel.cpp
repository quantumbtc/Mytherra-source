...
/** Call node.updateRwSetting() with Mytherra 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Mytherra 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/mytherra/mytherra/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because mytherra 22.x will not try to read these.
...
//! Convert enabled/size values to mytherra -prune setting.
...
//! Get pruning enabled value to show in GUI from mytherra -prune setting.
...
//! Get pruning size value to show in GUI from mytherra -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayMytherraUnit")) {
        settings.setValue("DisplayMytherraUnit", QVariant::fromValue(MytherraUnit::MYT));
    }
    QVariant unit = settings.value("DisplayMytherraUnit");
    if (unit.canConvert<MytherraUnit>()) {
        m_display_mytherra_unit = unit.value<MytherraUnit>();
    } else {
        m_display_mytherra_unit = MytherraUnit::MYT;
        settings.setValue("DisplayMytherraUnit", QVariant::fromValue(m_display_mytherra_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if mytherra.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_mytherra_unit);
...
{
    if (new_unit.isNull() || new_unit.value<MytherraUnit>() == m_display_mytherra_unit) return;
    m_display_mytherra_unit = new_unit.value<MytherraUnit>();
    QSettings settings;
    settings.setValue("DisplayMytherraUnit", QVariant::fromValue(m_display_mytherra_unit));
    Q_EMIT displayUnitChanged(m_display_mytherra_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/mytherra/mytherra/pull/8273
    ...
    // (https://github.com/mytherra-core/gui/issues/567).
