...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<MytherraUnit>("MytherraUnit");
#else
    qRegisterMetaType<MytherraUnit>("MytherraUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. mytherra_de.qm (shortcut "de" needs to be defined in mytherra.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. mytherra_de_DE.qm (shortcut "de_DE" needs to be defined in mytherra.qrc)
...
static const char* qt_argv = "mytherra-qt";

MytherraApplication::MytherraApplication()
...
void MytherraApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the MytherraApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", MytherraGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void MytherraApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool MytherraApplication::createOptionsModel(bool resetSettings)
{
...
void MytherraApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new MytherraGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &MytherraGUI::quitRequested, this, &MytherraApplication::requestShutdown);
...
void MytherraApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void MytherraApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool MytherraApplication::baseInitialize()
{
    return node().baseInitialize();
}

void MytherraApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &MytherraApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &MytherraApplication::handleRunawayException);
    connect(this, &MytherraApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &MytherraApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void MytherraApplication::parameterSetup()
{
...
void MytherraApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void MytherraApplication::requestInitialize()
{
...
void MytherraApplication::requestShutdown()
{
...
void MytherraApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // mytherra: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &MytherraGUI::handlePaymentRequest);
            connect(window, &MytherraGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void MytherraApplication::handleRunawayException(const QString &message)
{
...
void MytherraApplication::handleNonFatalException(const QString& message)
{
...
WId MytherraApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool MytherraApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", MytherraGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(mytherra);
    Q_INIT_RESOURCE(mytherra_locale);
    ...
    /// 6-7. Parse mytherra.conf, determine network, switch to network specific
    ...
    // mytherra: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
