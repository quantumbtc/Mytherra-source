...
#include <qt/mytherraaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Mytherra address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new MytherraAddressEntryValidator(parent));
    widget->setCheckValidator(new MytherraAddressCheckValidator(parent));
}
...
bool parseMytherraURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no mytherra: URI
    if(!uri.isValid() || uri.scheme() != QString("mytherra"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!MytherraUnits::parse(MytherraUnit::MYT, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseMytherraURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseMytherraURI(uriInstance, out);
}

QString formatMytherraURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("mytherra:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(MytherraUnits::format(MytherraUnit::MYT, info.amount, false, MytherraUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openMytherraConf()
{
    ...
    /* Open mytherra.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mytherra.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Mytherra (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Mytherra (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for Mytherra*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "mytherra.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("mytherra-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a mytherra.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Mytherra\n";
        else
            optionFile << strprintf("Name=Mytherra (%s)\n", chain);
