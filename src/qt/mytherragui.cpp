...
const std::string MytherraGUI::DEFAULT_UIPLATFORM =
...
MytherraGUI::MytherraGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &MytherraGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &MytherraGUI::showModalOverlay);
...
void MytherraGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Mytherra address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and mytherra: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &MytherraGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &MytherraGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &MytherraGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Mytherra addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Mytherra addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Mytherra Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Mytherra Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a mytherra: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Mytherra command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &MytherraGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &MytherraGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &MytherraGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &MytherraGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &MytherraGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &MytherraGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &MytherraGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &MytherraGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &MytherraGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &MytherraGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &MytherraGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &MytherraGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &MytherraGUI::showDebugWindow);
}

void MytherraGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void MytherraGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &MytherraGUI::setCurrentWalletBySelectorIndex);
...
void MytherraGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &MytherraGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &MytherraGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &MytherraGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &MytherraGUI::showProgress);
...
#ifdef ENABLE_WALLET
void MytherraGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void MytherraGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &MytherraGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &MytherraGUI::removeWallet);
...
WalletController* MytherraGUI::getWalletController()
{
    return m_wallet_controller;
}

void MytherraGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &MytherraGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &MytherraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &MytherraGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &MytherraGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &MytherraGUI::incomingTransaction);
    connect(this, &MytherraGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void MytherraGUI::removeWallet(WalletModel* walletModel)
{
...
void MytherraGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void MytherraGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void MytherraGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void MytherraGUI::setWalletActionsEnabled(bool enabled)
{
...
void MytherraGUI::createTrayIcon()
{
...
void MytherraGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &MytherraGUI::toggleHidden);
...
void MytherraGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void MytherraGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void MytherraGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void MytherraGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void MytherraGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void MytherraGUI::openClicked()
{
...
void MytherraGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void MytherraGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void MytherraGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void MytherraGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void MytherraGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void MytherraGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void MytherraGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void MytherraGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Mytherra network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void MytherraGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void MytherraGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void MytherraGUI::updateHeadersSyncProgressLabel()
{
...
void MytherraGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void MytherraGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &MytherraGUI::quitRequested);
...
void MytherraGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void MytherraGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void MytherraGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &MytherraGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &MytherraGUI::show);
                e->ignore();
            }
        }
    }
...
void MytherraGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void MytherraGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void MytherraGUI::incomingTransaction(const QString& date, MytherraUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(MytherraUnits::formatWithUnit(unit, amount, true));
...
void MytherraGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void MytherraGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool MytherraGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool MytherraGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void MytherraGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void MytherraGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void MytherraGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void MytherraGUI::updateProxyIcon()
{
    std::string ip_port;
...
void MytherraGUI::updateWindowTitle()
{
...
void MytherraGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void MytherraGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void MytherraGUI::detectShutdown()
{
...
void MytherraGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void MytherraGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(MytherraGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = MytherraGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void MytherraGUI::subscribeToCoreSignals()
{
...
void MytherraGUI::unsubscribeFromCoreSignals()
{
...
bool MytherraGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<MytherraUnit> units = MytherraUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const MytherraUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, MytherraUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const MytherraUnit u : MytherraUnits::availableUnits()) {
        menu->addAction(MytherraUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(MytherraUnit newUnits)
{
    setText(MytherraUnits::longName(newUnits));
}
