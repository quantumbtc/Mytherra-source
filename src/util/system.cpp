...
const char * const MYTHERRA_CONF_FILENAME = "mytherra.conf";
const char * const MYTHERRA_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //mytherra-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", MYTHERRA_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Mytherra
    // macOS: ~/Library/Application Support/Mytherra
    // Unix-like: ~/.mytherra
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mytherra";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Mytherra";
#else
    // Unix-like
    return pathRet / ".mytherra";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", MYTHERRA_CONF_FILENAME));
}
