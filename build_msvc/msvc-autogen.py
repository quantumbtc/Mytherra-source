...
def parse_config_into_myt_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Mytherra Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/mytherra_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/mytherra_config.h'), "w", encoding="utf8") as myt_config:
        myt_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Mytherra-core msbuild configuration initialiser.')
    ...
    parse_config_into_myt_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/mytherra_config.h'), os.path.join(SOURCE_DIR, 'config/mytherra-config.h'))
