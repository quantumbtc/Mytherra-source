# Bootstrappable Mytherra Core Builds

This directory contains the files necessary to perform bootstrappable Mytherra
Core builds.
...
To build Mytherra Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Mytherra Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/mytherra-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/mytherra-core/guix.sigs).
