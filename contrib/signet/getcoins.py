...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing mytherra-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='mytherra-cli', help='mytherra-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Mytherra address to which the faucet should send')
...
parser.add_argument('mytherra_cli_args', nargs='*', help='Arguments to pass on to mytherra-cli (default: -signet)')

args = parser.parse_args()

if args.mytherra_cli_args == []:
    args.mytherra_cli_args = ['-signet']


def mytherra_cli(rpc_command_and_params):
    argv = [args.cmd] + args.mytherra_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = mytherra_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = mytherra_cli(['getnewaddress', 'faucet', 'bech32'])
