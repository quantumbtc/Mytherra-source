...
""" Example logging Mytherra Core mempool events using the mempool:added,
...
def main(mytherrad_path):
    mytherrad_with_usdts = USDT(path=str(mytherrad_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    mytherrad_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    mytherrad_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    mytherrad_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    mytherrad_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[mytherrad_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/mytherrad")
        exit(1)
